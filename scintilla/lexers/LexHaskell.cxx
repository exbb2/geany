...
void SCI_METHOD LexerHaskell::Lex(unsigned int startPos, int length, int initStyle
   ...
   while (sc.More()) {
      ...
      if (sc.state == SCE_HA_OPERATOR) {
         ...
         if (options.stylingWithinPreprocessor && !IsHaskellLetter(sc.ch)) {
            sc.SetState(SCE_HA_DEFAULT);
         } else if (sc.atLineEnd) {
            sc.SetState(SCE_HA_DEFAULT);
            sc.Forward(); // prevent double counting a line
...
unipred(u_IsHaskellSymbol,
   (    NUMCAT_PC  // ConnectorPunctuation
      | NUMCAT_PD  // DashPunctuation
      | NUMCAT_PO  // OtherPunctuation
      | NUMCAT_SM  // MathSymbol
      | NUMCAT_SC  // CurrencySymbol
      | NUMCAT_SK  // ModifierSymbol
      | NUMCAT_SO  // OtherSymbol
   )
   )
